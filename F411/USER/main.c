...
#define SD_CYCLE_TIME 470  // about 21.27K Sample rate
//#define SD_CYCLE_TIME 500  // about 20K Sample rate
...
volatile uint32_t sdtemp_cnt=0;	
volatile uint32_t seed = 0;
volatile uint32_t time1,time2,time3,time4=0;
volatile uint32_t t1,t2,t3,t4,t5,t6,t7,t8,t9,t10,t11,t12,t13,t14,t15,t16,t17,t18,t19,t20=0;
volatile uint32_t real_sd_cycle = 0;
...
void TIM2_Init(void) {
    ...
    TIM2->PSC = 5; // Prescaler (50 MHz / 0 = 10MHz)
...
/****************************cycle call 32 channel*********************/	
			...
			/*******************************insert sd cmd during sample cycles***************************************/
			if(sd_cycle == SD_CYCLE_SOTRE-61)  // only can run below code after 80% sample cycle finish
			{
			...
			else if(sd_cycle ==SD_CYCLE_SOTRE-59)
			{
			...
			else if(sd_cycle == SD_CYCLE_SOTRE-57)
			{
				if (StopCondition==1&&TransferEnd==1)  // TransferEnd will set to 0 in interrupt 
				{  
			...
			else if(sd_cycle == SD_CYCLE_SOTRE-45)
			{
				/*******************************check card status***************************************/	
			...
			else if(sd_cycle == SD_CYCLE_SOTRE-30)
			{
		/*******************************check card status***************************************/	
		...
		/*******************************send dataconfig and cmd***************************************/	
			...
			else if(sd_cycle == SD_CYCLE_SOTRE-1)  // the last cycle record finish (now sdtemp_cnt should == 33*124)
			{
				/*******************************start DMA***************************************/		
				TransferError=SD_OK;
