...
u8 CardType=SDIO_STD_CAPACITY_SD_CARD_V1_1;		//SD卡类型（默认为1.x卡）
u32 CSD_Tab[4],CID_Tab[4],RCA=0;					//SD卡CSD,CID以及相对地址(RCA)数据
u8 DeviceMode=SD_DMA_MODE;		   				//工作模式,注意,工作模式必须通过SD_SetDeviceMode,后才算数.这里只是定义一个默认的模式(SD_DMA_MODE)
u8 StopCondition=0; 								//是否发送停止传输标志位,DMA多块读写的时候用到  
...
SD_Error SD_Init(void)
{
  ...
  GPIO_Init(GPIOA, &GPIO_InitStructure);//PD2复用功能输出
	
	 //引脚复用映射设置
	GPIO_PinAFConfig(GPIOB,GPIO_PinSource7,GPIO_AF_SDIO); //PC8,AF12
  ...
  GPIO_PinAFConfig(GPIOA,GPIO_PinSource6,GPIO_AF_SDIO);	
...
//blksize:块大小	  
//返回值:错误状态
uint32_t time11,time22,time33,time44,time55,time66=0;
SD_Error SD_WriteBlock_nowait(u8 *buf,long long addr,  u16 blksize)
{
	SD_Error errorstatus = SD_OK;
...
{		 
  ...
  DMA_InitTypeDef  DMA_InitStructure;
	...
	DMA_Cmd(DMA2_Stream3, DISABLE); // disable first
...
SD_Error SD_WriteMultiBlocks_nowait(u8 *buf,long long addr,u16 blksize,u32 nblks)
{
	SD_Error errorstatus = SD_OK;
	u8  power = 0, cardstate = 0;
	u32 timeout=0,bytestransferred=0;
...
u8 SD_WriteDisk_nowait(u8*buf,u32 sector,u8 cnt)
{
	u8 sta=SD_OK;
	u8 n;
	long long lsector=sector;
		...
		if((u32)buf%4!=0)
	{
		...
		{
			memcpy(SDIO_DATA_BUFFER,buf,512);
		 	sta=SD_WriteBlock_nowait(SDIO_DATA_BUFFER,lsector+512*n,512);//单个sector的写操作
			buf+=512;
		} 
	}else
	{
		if(cnt==1)sta=SD_WriteBlock_nowait(buf,lsector,512);    	//单个sector的写操作
		else sta=SD_WriteMultiBlocks_nowait(buf,lsector,512,cnt);	//多个sector  
	}
